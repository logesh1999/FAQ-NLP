...
def main():
    ...
    df = pd.read_csv(r'FAQ final.csv')
    ...
    if st.button('Search'):
        ...
        if s!= "" :
            ...
            if s not in df.iloc[index_sim, 0]:
                 st.write("Go to google [link](https://www.google.co.in/search?q="+s.replace(' ','%20')+"&source=null)")                     
        
        
    

    with open(r"FAQ final.CSV", encoding='utf-8') as d:
        data = d
        st.write('--------------------------------------')
